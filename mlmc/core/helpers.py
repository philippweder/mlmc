...
def mlmc_pilot(
    ...
    **payoff_kwargs
    ...
    for h in h_values:
        ...
        variances.append(result["var_diff"]) #this quantity is Var( Y_fine - Y_coarse)/n_samp

    biases = np.array(biases)
    variances = np.array(variances)
    
    E0 = np.polyfit(np.arange(1, nlevels), biases[1:] * (2 ** np.arange(1,nlevels)), 0)[-1]
    V0 = np.polyfit(
        np.arange(1,nlevels), variances[1:] * nsamp* (2.0 **(np.arange(1,nlevels))), 0
    )[-1]
